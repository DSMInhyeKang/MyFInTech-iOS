...
class DepositDetailViewController: UIViewController {
    ...
    var deposit = DepositEntity(id: 0, type: .MMDA, name: "", company: "", maxRate: "", defaultRate: "", target: "", detail: "", interest: "", url: "", isProtected: true, isBankingSector: true)
    ...
    func bind() {
        ...
        linkButton.rx.tap
            .subscribe(onNext: {
                if let url = URL(string: self.deposit.url) { UIApplication.shared.open(url, options: [:]) }
            }).disposed(by: disposeBag)
        nameLabel.text = deposit.name
        protectionImageView.isHidden = deposit.isProtected ? false : true
        companyImageView.image = deposit.company.convertCompany()
        companyLabel.text = deposit.company
        maxRateLabel.text = deposit.maxRate
        defaultRateLabel.text = deposit.defaultRate
        detailTextView.text = deposit.detail
        targetTextView.text = deposit.target
        interestTextView.text = deposit.interest
    }
